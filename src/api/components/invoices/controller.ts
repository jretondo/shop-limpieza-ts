...
export = (injectedStore: typeof StoreType) => {
  ...
  const cajaList = async (
    ...
    if (page) {
      ...
      const totalesCosto = await store.list(
        Tables.FACTURAS,
        [`SUM(${Columns.facturas.total_compra}) AS SUMA`],
        filters,
      );
      ...
      if (pdf) {
        const cajaList = await createListSellsPDF(
          userId,
          ptoVtaId,
          desde,
          hasta,
  ...
  const insertFact = async (
    ...
    if (Number(newFact.forma_pago) === 2) {
      newFact.total_compra = Number(
        (newFact.total_compra + newFact.total_fact * 0.08).toFixed(2),
      );
    }
      ...
      const rows: Promise<Array<Array<any>>> = new Promise(
        (resolve, reject) => {
          ...
          newDetFact.map(async (item, key) => {
            if (Number(newFact.forma_pago) === 2) {
              item.total_costo = Number(
                (item.total_costo + item.total_prod * 0.08).toFixed(2),
              );
            }
            const values = [];
            values.push(factId);
